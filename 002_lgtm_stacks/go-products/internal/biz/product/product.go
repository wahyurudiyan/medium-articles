...
func (uc *usecase) CreateProducts(ctx context.Context, products []productRepo.CreateProductsParams) error {
	...
	log.Context(ctx).Debugw("Creating products with batch")
	...
	for i := 0; i < len(products); i += batchSize {
		...
		log.Context(ctx).Debugw("Creating products", map[string]interface{}{
...
func (uc *usecase) FetchProducts(ctx context.Context, page int32, limit int32) ([]productRepo.Product, error) {
	...
	log.Context(ctx).Debugw("Fetching list of products", map[string]interface{}{
	...
	if err != nil {
		log.Context(ctx).Errorw("Error occur when updating product by ID!", map[string]interface{}{
			"page": page, "page_size": limit,
		})
		return nil, err
	}

	log.Context(ctx).Debugw("Products successfully loaded", map[string]interface{}{
		"page": page, "page_size": limit,
	})
...
func (uc *usecase) FetchProductByID(ctx context.Context, id int64) (*productRepo.Product, error) {
	...
	log.Context(ctx).Debugw("Fetching product (by ID)", map[string]interface{}{
		"product.id": id,
	})

	product, err := uc.productRepo.ReadProductByID(ctx, id)
	if err != nil {
		log.Context(ctx).Errorw("Error occur when reading product by ID!", map[string]interface{}{
			"product.sku": product.Sku, "product.name": product.Name,
		})
		return nil, err
	}

	log.Context(ctx).Debugw("Product found!", map[string]interface{}{
		"product.id": id,
	})
...
func (uc *usecase) FetchProductBySKU(ctx context.Context, sku string) (*productRepo.Product, error) {
	...
	log.Context(ctx).Debugw("Fetching product (by SKU)", map[string]interface{}{
		"product.sku": sku,
	})

	product, err := uc.productRepo.ReadProductBySKU(ctx, sku)
	if err != nil {
		log.Context(ctx).Errorw("Error occur when reading product by SKU!", map[string]interface{}{
			"product.sku": product.Sku, "product.name": product.Name,
		})
		return nil, err
	}
...
func (uc *usecase) UpdateProduct(ctx context.Context, product *productRepo.Product) (*productRepo.Product, error) {
	...
	log.Context(ctx).Debugw("Updating product", map[string]interface{}{
		"product.sku": product.Sku, "product.name": product.Name,
	})
	...
	if err != nil {
		log.Context(ctx).Errorw("Error occur when updating product!", map[string]interface{}{
			"product.sku": product.Sku, "product.name": product.Name,
		})
		return nil, err
	}

	log.Context(ctx).Debugw("Product updated successfully!", map[string]interface{}{
		"product.sku": product.Sku, "product.name": product.Name,
	})
...
func (uc *usecase) DeleteProduct(ctx context.Context, sku string) error {
	...
	log.Context(ctx).Debugw("Process deleting product", map[string]interface{}{
		"sku": sku,
	})

	_, err := uc.productRepo.SoftDeleteProduct(ctx, sku)
	if err != nil {
		log.Context(ctx).Errorw("Error occur when deleting product!", map[string]interface{}{
			"error": err, "product.sku": sku,
		})
		return err
	}

	log.Context(ctx).Debugw("Product deleted successfully!", map[string]interface{}{
		"sku": sku,
	})
